export const SERVICES = [
  ...
  { name: 'Other (Custom)', rate: 0, type: 'custom' },
]

// Index of the "Other (Custom)" entry above — a flat, typed-in amount and
// description instead of a rate * qty calculation.
export const CUSTOM_SERVICE_IDX = SERVICES.length - 1
...
// Preset options shown in the pet "Type" picker; anything else typed in
// (hamster, bird, etc.) is stored as free text and treated like "Other".
export const PET_TYPES = ['Dog', 'Cat', 'Goldfish', 'Other']
...
export function calcLineTotal(svcIdx, qty, customAmount) {
  if (svcIdx < 1 || !SERVICES[svcIdx]) return 0
  const s = SERVICES[svcIdx]
  if (s.type === 'custom') return parseFloat(customAmount) || 0
...
export function getRateLabel(svcIdx) {
  if (svcIdx < 1) return ''
  const s = SERVICES[svcIdx]
  if (s.type === 'custom') return 'Custom amount'
...
export function getDefaultQty(svcIdx) {
  ...
  const s = SERVICES[svcIdx]
  if (s.type === 'custom') return ''
  return s.type === 'time' ? '30' : '1'
}

// The label shown for a line item's service — for a custom entry this is
// whatever the user typed in (e.g. "Nail trim"), falling back to the
// service's own name if no description was given.
export function getServiceLabel(svcIdx, customDescription) {
  const s = SERVICES[svcIdx]
  if (!s) return ''
  if (s.type === 'custom') return (customDescription && customDescription.trim()) || s.name
  return s.name
}
...
export function buildShareText(inv) {
  ...
  const lines = rows.map(r => {
    ...
    return `• ${getServiceLabel(r.service_idx, r.custom_description)}${date ? ` – ${date}` : ''}: $${tot.toFixed(2)}`
  }).join('\n')
  ...
  return [
    `🐾 Love 4 Dogs — Invoice #${inv.invoice_number}`,
    `Client: ${inv.client_name}${inv.dog_names ? ` | ${inv.dog_names}` : ''}`,
...
export function buildInvoiceHTML(inv) {
  ...
  const rowsHTML = rows.map(r => {
    ...
    return `<tr>
      <td>${getServiceLabel(r.service_idx, r.custom_description)}<br><small style="color:#888">${getRateLabel(r.service_idx)}</small></td>
      <td>${r.date ? formatDateShort(r.date) : ''}</td>
      <td>${isCustom ? '' : `${r.qty} ${getQtyLabel(r.service_idx)}`}</td>
  ...
  if (!matched) {
    for (const c of clients) {
      if (text.includes(c.name.toLowerCase())) {
        ...
        } else if (c.dogs?.length > 1) {
          // No specific dog mentioned — default to all of this client's dogs
          result.dog_name = c.dogs.map(d => d.name).join(', ')
        }
        break
      }
    }
