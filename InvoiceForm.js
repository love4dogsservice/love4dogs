...
export default function InvoiceDetail({ inv, onEdit, onBack, onTogglePaid, onDelete }) {
  ...
  const handleText = () => {
    window.location.href = `sms:?body=${encodeURIComponent(buildShareText(inv))}`
  }

  const handleCopy = async () => {
    try { await navigator.clipboard.writeText(buildShareText(inv)); showToast('Copied!') }
    catch { showToast('Copy failed') }
  }

  const filledItems = (inv.line_items || []).filter(r => r.service_idx > 0)
  const periodStr = inv.period_start ? `${inv.period_start}${inv.period_end ? ' – ' + inv.period_end : ''}` : ''

  return (
    <div style={{ background: '#e8f4fd', minHeight: '100vh' }}>
      {toast && <Toast msg={toast} />}

      <div className="no-print" style={{ background: COLORS.blue, padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
        <button onClick={onBack} style={{ background: 'none', border: 'none', color: '#fff', fontSize: '1.2rem' }}>←</button>
        <div style={{ color: '#fff', fontWeight: 900, fontSize: '1rem' }}>Invoice #{inv.invoice_number}</div>
        <button onClick={onEdit} style={{ background: 'none', border: 'none', color: '#fff', fontSize: '0.85rem', fontWeight: 700 }}>Edit</button>
      </div>
      ...
      <div style={{ padding: '16px', maxWidth: 700, margin: '0 auto' }}>
        ...
        <div className="no-print" style={{ background: '#fff', borderRadius: 14, padding: '14px 16px', marginBottom: 12, boxShadow: '0 2px 10px rgba(0,0,0,0.07)' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
            <div>
              <div style={{ fontWeight: 900, color: COLORS.navy, fontSize: '1.1rem' }}>{inv.client_name}</div>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ display: 'inline-block', marginTop: 4, background: inv.paid ? COLORS.lightGreen : COLORS.lightRed, color: inv.paid ? COLORS.green : COLORS.coral, fontSize: '0.65rem', fontWeight: 800, padding: '3px 10px', borderRadius: 10, textTransform: 'uppercase' }}>
                {inv.paid ? 'Paid' : 'Unpaid'}
              </div>
            </div>
          </div>
          <button onClick={handleTogglePaid} disabled={toggling}
            style={{ width: '100%', padding: '11px', borderRadius: 10, border: 'none', background: inv.paid ? COLORS.lightRed : COLORS.lightGreen, color: inv.paid ? COLORS.coral : COLORS.green, fontWeight: 800, fontSize: '0.9rem' }}>
        ...
        <div className="no-print" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 8, marginBottom: 12 }}>
          {[['✉️','Email',handleEmail],['💬','Text',handleText],['📋','Copy',handleCopy],['🖨️','Print',() => window.print()]].map(([icon,label,action]) => (
            <button key={label} onClick={action} style={{ background: '#fff', border: 'none', borderRadius: 12, padding: '12px 4px', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.07)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 14, overflow: 'hidden', boxShadow: '0 2px 10px rgba(0,0,0,0.07)', marginBottom: 12 }}>
          <div style={{ background: COLORS.blue, padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ background: COLORS.coral, color: '#fff', fontWeight: 900, fontSize: '0.85rem', padding: '5px 14px', borderRadius: 8, letterSpacing: 1 }}>#{inv.invoice_number}</div>
          </div>
          ...
          <div style={{ padding: '14px 20px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 16px', marginBottom: 14, paddingBottom: 14, borderBottom: `1px solid ${COLORS.lightBlue}` }}>
              <div><div style={{ fontSize: '0.62rem', color: COLORS.coral, fontWeight: 800, textTransform: 'uppercase' }}>Client</div><div style={{ fontWeight: 700, color: COLORS.navy }}>{inv.client_name}</div></div>
              {inv.dog_names && <div><div style={{ fontSize: '0.62rem', color: COLORS.coral, fontWeight: 800, textTransform: 'uppercase' }}>Dog(s)</div><div style={{ fontWeight: 700, color: COLORS.navy }}>{inv.dog_names}</div></div>}
              {periodStr && <div style={{ gridColumn: '1/-1' }}><div style={{ fontSize: '0.62rem', color: COLORS.coral, fontWeight: 800, textTransform: 'uppercase' }}>Period</div><div style={{ fontWeight: 600, color: '#333' }}>{periodStr}</div></div>}
            </div>

            {inv.special_notes && <div style={{ background: COLORS.lightBlue, borderRadius: 8, padding: '8px 12px', marginBottom: 12, fontSize: '0.82rem', color: COLORS.navy }}>📝 {inv.special_notes}</div>}
                ...
                {filledItems.map((r, i) => {
                  ...
                  return (
                    <tr key={i} style={{ borderBottom: '1px dashed #eee' }}>
                      ...
                      <td style={{ padding: '8px 8px', color: '#555', fontSize: '0.78rem' }}>{r.date ? formatDateShort(r.date) : ''}</td>
            ...
            {inv.payment_notes && <div style={{ marginTop: 12, padding: '10px 12px', border: '1px solid #eee', borderRadius: 8, fontSize: '0.8rem', color: '#555' }}>{inv.payment_notes}</div>}
          </div>
        ...
        <div className="no-print">
          {!confirmDelete ? (
            <button onClick={() => setConfirmDelete(true)} style={{ width: '100%', background: 'none', border: '1px solid #ffbbb0', color: '#c94428', padding: '10px', borderRadius: 10, fontSize: '0.85rem', fontWeight: 700 }}>Delete Invoice</button>
          ) : (
